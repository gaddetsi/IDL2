...
def to_categorical_per_hour(y, num_classes):
    """
    ...
    class_ = y[:, 0]*2 + y[:, 1]//30 # if there are more than 30 minutes, it is in the next class

    return keras.utils.to_categorical(class_, num_classes)

# Convert labels to one-hot encoding
y_train = to_categorical_per_hour(y_train, num_classes)
y_val = to_categorical_per_hour(y_val, num_classes)
y_test = to_categorical_per_hour(y_test, num_classes)
...
# hier this function does not work for fitting
